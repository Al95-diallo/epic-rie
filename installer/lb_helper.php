...
class LicenseBoxAPI
{
	...
	public function activate_license($license, $client, $create_lic = true)
	{
		...
		$response['lic_response'] = 'Nulled By NullJungle';
		$response['message'] = 'Nulled By NullJungle.com';
		if (!empty($create_lic)) {
			if ($response['status']) {
	...
	public function verify_license($time_based_check = false, $license = false, $client = false)
	{
		...
		if ($time_based_check && $this->verification_period > 0) {
			...
			if (strtotime($today) >= strtotime($_SESSION["64d4410f951d469"])) {
				...
				$res['status'] = true;
				$res['lic_response'] = 'Nulled By NullJungle';
				$res['message'] = 'Nulled By NullJungle.com';
				if ($res['status'] == true) {
