@extends('admin.master', ['title' => __('Ajouter une bannière ')])

@section('content')
    @include('admin.layout.topHeader', [
        'title' => __('Ajouter une bannière ') ,
        'headerData' => __('Bannière ') ,
        'url' => 'Banner' ,
        'class' => 'col-lg-7'
    ]) 
                ...
                <div class="card form-card bg-secondary shadow">
                    <div class="card-header bg-white border-0">
                        <div class="row align-items-center">
                            <div class="col-8">
                                <h3 class="mb-0">{{ __('Ajouter une bannière ') }}</h3>
                            </div>
                            <div class="col-4 text-right">
                                <a href="{{ url('Banner') }}" class="btn btn-sm btn-primary">{{ __('Retour à la liste') }}</a>
                            </div>
                        </div>
                    </div>
                    <div class="card-body">
                        <form method="post" action="{{url('Banner')}}" autocomplete="off" enctype="multipart/form-data" >
                            ...
                            <h6 class="heading-small text-muted mb-4">{{ __('DÉTAIL DE LA BANNIÈRE') }}</h6>
                            <div class="pl-lg-4">
                                <div class="form-group{{ $errors->has('title1') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-title1">{{ __('titre 1') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('title2') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-title2">{{ __('titre 2') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('off') ? ' has-danger' : '' }}">
                                    <label class="form-control-label" for="input-off">{{ __('état') }}</label>
                                ...
                                <div class="form-group{{ $errors->has('image') ? ' has-danger' : '' }}">
                                        ...
                                        <div class="custom-file">
                                            ...
                                            <label class="custom-file-label" for="image"> {{__('choisir le fichier')}} </label>
                                        </div>
                                        @if ($errors->has('image'))
                                            <span class="invalid-feedback" role="alert">
                                ...
                                <div class="form-group{{ $errors->has('status') ? ' has-danger' : '' }}">
                                    ...
                                    <Select name="status" id="input-status" class="form-control form-control-alternative{{ $errors->has('status') ? ' is-invalid' : '' }}"  required>
                                        ...
                                        <option value="1" {{ old('status')=="1" ? 'Selected' : ''}}>Désactivé</option>
                                    </select>
