...
<div class="bs-canvas bs-canvas-left position-fixed bg-cart h-100" id="cart">
	<div class="bs-canvas-header side-cart-header p-3 ">
		<div class="d-inline-block  main-cart-title">
			{{__("Mon pannier")}}<span>(<span class="cart-item-count m-0">{{count((array) session('cart'))}}</span> Articles)</span>
		</div>
	...
	<div class="bs-canvas-footer">
		@if ($discount != 0)
			<div class="cart-total-dil saving-total">
				<h4>{{__('remise')}}</h4>
				<span><span class="cart-discount">{{$discount}}</span>{{$data['currency']}}</span>
			</div>
		@endif
		<div class="cart-total-dil saving-total">
			<h4>{{__('Total')}}</h4>
			<span><span class="cart-saving-total">{{$saving}}</span>{{$data['currency']}}</span>
		</div>
		<div class="main-total-cart">
			<h2> {{__('Total')}}</h2>
			<span><span class="cart-final-total">{{$total + $setting->delivery_charge - $discount}}</span>{{$data['currency']}}</span>
		</div>
		<div class="checkout-cart">
			<a href="{{url('/checkout')}}" class="cart-checkout-btn hover-btn"> {{__('passé à la caisse')}} </a>
		</div>
	</div>
</div>
