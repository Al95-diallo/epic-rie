...
@section('content')
    @include('admin.layout.topHeader', [
        'title' => __('Ajouter une sous-catégorie d\'épicerie') ,
        'headerData' => __('Sous-catégories') ,
                        ...
                        <div class="card form-card bg-secondary shadow">
                            <div class="card-header bg-white border-0">
                                <div class="row align-items-center">
                                    <div class="col-8">
                                        <h3 class="mb-0">{{ __('Ajouter une sous-catégorie d\'épicerie') }}</h3>
                                    </div>
                                    <div class="col-4 text-right">
                                        <a href="{{ url('GrocerySubCategory') }}" class="btn btn-sm btn-primary">{{ __('Retour à la liste') }}</a>
                                    </div>
                                </div>
                            </div>
                            <div class="card-body">
                                <form method="post" action="{{url('GrocerySubCategory')}}" class="grocery_subcategory" autocomplete="off" enctype="multipart/form-data" >
                                    ...
                                    <h6 class="heading-small text-muted mb-4">{{ __('DÉTAIL DE LA SOUS-CATÉGORIE') }}</h6>
                                    <div class="pl-lg-4">
                                        ...
                                        <div class="form-group{{ $errors->has('category_id') ? ' has-danger' : '' }}">
                                            <label class="form-control-label" for="input-category_id">{{ __('Catégories') }}</label>
